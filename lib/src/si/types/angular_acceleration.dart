...
class AngularAcceleration extends Quantity {
  ...
  static final AngularAccelerationUnits radiansPerSecondSquared =
      AngularAccelerationUnits.anglePerTimeSquared(Angle.radians, Time.seconds);

  /// Accepted for use with the SI.
  static final AngularAccelerationUnits degreesPerSecondSquared =
      AngularAccelerationUnits.anglePerTimeSquared(Angle.degrees, Time.seconds);
}
...
class AngularAccelerationUnits extends AngularAcceleration with Units {
  ...
  /// Constructs an instance from angle and time units.
  AngularAccelerationUnits.anglePerTimeSquared(AngleUnits au, TimeUnits tu)
      : super.misc(au.valueSI / (tu.valueSI * tu.valueSI)) {
    name = '${au.name} per ${tu.singular} squared';
    singular = '${au.singular} per ${tu.singular} squared';
    convToMKS = au.valueSI / (tu.valueSI * tu.valueSI);
    abbrev1 = au.abbrev1 != null && tu.abbrev1 != null
        ? '${au.abbrev1} / ${tu.abbrev1}\u{00b2}'
        : null;
    abbrev2 = au.abbrev2 != null && tu.abbrev2 != null
        ? '${au.abbrev2} ${tu.abbrev2}\u{207b}\u{00b2}'
        : null;
    metricBase = false;
    offset = 0.0;
  }
