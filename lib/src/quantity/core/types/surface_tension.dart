...
class SurfaceTension extends Quantity {
  ...
  static const Dimensions surfaceTensionDimensions =
      const Dimensions.constant(const {"Mass": 1, "Time": -2});

  /// The standard SI unit.
  static final SurfaceTensionUnits newtonsPerMeter =
      new SurfaceTensionUnits.forceLength(Force.newtons, Length.meters);

  SurfaceTension({dynamic N_per_m, double uncert: 0.0})
      : super(N_per_m != null ? N_per_m : 0.0, SurfaceTension.newtonsPerMeter,
            uncert);

  SurfaceTension._internal(conv)
      : super._dimensions(conv, SurfaceTension.surfaceTensionDimensions);

  /// Constructs a SurfaceTension based on the [value]
  /// and the conversion factor intrinsic to the passed [units].
  ///
  SurfaceTension.inUnits(value, SurfaceTensionUnits units,
      [double uncert = 0.0])
      : super(value, units != null ? units : SurfaceTension.newtonsPerMeter,
            uncert);

  const SurfaceTension.constant(Number valueSI,
      {SurfaceTensionUnits units, num uncert: 0.0})
      : super.constant(
...
class SurfaceTensionUnits extends SurfaceTension with Units {
  ...
  SurfaceTensionUnits.forceLength(ForceUnits fu, LengthUnits lu)
      : super._internal(fu.valueSI / lu.valueSI) {
    this.name = "${fu.name} per ${lu.singular}";
    this.singular = "${fu.singular} per ${lu.singular}";
    this._convToMKS = fu.valueSI / lu.valueSI;
    this._abbrev1 = fu._abbrev1 != null && lu._abbrev1 != null
        ? "${fu._abbrev1} / ${lu._abbrev1}"
        : null;
    this._abbrev2 = fu._abbrev2 != null && lu._abbrev2 != null
        ? "${fu._abbrev2}/${lu._abbrev2}"
        : null;
