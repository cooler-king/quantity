...
class SpecificHeatCapacity extends Quantity {
  ...
  static const Dimensions specificHeatCapacityDimensions =
      const Dimensions.constant(
          const {"Length": 2, "Time": -2, "Temperature": -1});

  /// The standard SI unit.
  static final SpecificHeatCapacityUnits joulesPerKilogramKelvin =
      new SpecificHeatCapacityUnits.energyMassTemperature(
          Energy.joules, Mass.kilograms, TemperatureInterval.kelvins);

  SpecificHeatCapacity({dynamic J_per_kgK, double uncert: 0.0})
      : super(J_per_kgK != null ? J_per_kgK : 0.0,
            SpecificHeatCapacity.joulesPerKilogramKelvin, uncert);

  SpecificHeatCapacity._internal(conv)
      : super._dimensions(
            conv, SpecificHeatCapacity.specificHeatCapacityDimensions);

  /// Constructs a SpecificHeatCapacity based on the [value]
  /// and the conversion factor intrinsic to the passed [units].
  ///
  SpecificHeatCapacity.inUnits(value, SpecificHeatCapacityUnits units,
      [double uncert = 0.0])
      : super(
            value,
            units != null
                ? units
                : SpecificHeatCapacity.joulesPerKilogramKelvin,
            uncert);

  const SpecificHeatCapacity.constant(Number valueSI,
      {SpecificHeatCapacityUnits units, num uncert: 0.0})
...
class SpecificHeatCapacityUnits extends SpecificHeatCapacity with Units {
  ...
  SpecificHeatCapacityUnits.energyMassTemperature(
    ...
    this._abbrev1 = eu._abbrev1 != null && mu._abbrev1 != null
        ? "${eu._abbrev1} / ${mu._abbrev1} ${tu._abbrev1}"
        : null;
    this._abbrev2 = eu._abbrev2 != null && mu._abbrev2 != null
        ? "${eu._abbrev2}${mu._abbrev2}${tu._abbrev2}"
        : null;
