...
class ThermalConductivity extends Quantity {
  ...
  static final ThermalConductivityUnits wattsPerMeterKelvin =
      new ThermalConductivityUnits.powerLengthTemperature(
          Power.watts, Length.meters, TemperatureInterval.kelvins);

  ThermalConductivity({dynamic W_per_mK, double uncert: 0.0})
      : super(W_per_mK != null ? W_per_mK : 0.0,
            ThermalConductivity.wattsPerMeterKelvin, uncert);

  ThermalConductivity._internal(conv)
      : super._dimensions(
            conv, ThermalConductivity.thermalConductivityDimensions);

  /// Constructs a ThermalConductivity based on the [value]
  /// and the conversion factor intrinsic to the passed [units].
  ///
  ThermalConductivity.inUnits(value, ThermalConductivityUnits units,
      [double uncert = 0.0])
      : super(
            value,
            units != null ? units : ThermalConductivity.wattsPerMeterKelvin,
            uncert);

  const ThermalConductivity.constant(Number valueSI,
      {ThermalConductivityUnits units, num uncert: 0.0})
...
class ThermalConductivityUnits extends ThermalConductivity with Units {
  ...
  ThermalConductivityUnits.powerLengthTemperature(
    ...
    this._abbrev1 = pu._abbrev1 != null && lu._abbrev1 != null
        ? "${pu._abbrev1} / ${lu._abbrev1} ${tiu._abbrev1}"
        : null;
    this._abbrev2 = pu._abbrev2 != null && lu._abbrev2 != null
        ? "${pu._abbrev2}/${lu._abbrev2}${tiu._abbrev2}"
        : null;
