...
class ElectricFluxDensity extends Quantity {
  ...
  static final ElectricFluxDensityUnits coulombsPerSquareMeter =
      new ElectricFluxDensityUnits.chargeArea(
          Charge.coulombs, Area.squareMeters);

  ElectricFluxDensity({dynamic C_per_m2, double uncert: 0.0})
      : super(C_per_m2 != null ? C_per_m2 : 0.0,
            ElectricFluxDensity.coulombsPerSquareMeter, uncert);

  ElectricFluxDensity._internal(conv)
      : super._dimensions(
            conv, ElectricFluxDensity.electricFluxDensityDimensions);

  /// Constructs a ElectricFluxDensity based on the [value]
  /// and the conversion factor intrinsic to the passed [units].
  ///
  ElectricFluxDensity.inUnits(value, ElectricFluxDensityUnits units,
      [double uncert = 0.0])
      : super(
            value,
            units != null ? units : ElectricFluxDensity.coulombsPerSquareMeter,
            uncert);

  const ElectricFluxDensity.constant(Number valueSI,
      {ElectricFluxDensityUnits units, num uncert: 0.0})
...
class ElectricFluxDensityUnits extends ElectricFluxDensity with Units {
  ...
  ElectricFluxDensityUnits.chargeArea(ChargeUnits ecu, AreaUnits au)
      : super._internal(ecu.valueSI * au.valueSI) {
    this.name = "${ecu.name} per ${au.singular}";
    this.singular = "${ecu.singular} per ${au.singular}";
    this._convToMKS = ecu.valueSI * au.valueSI;
    this._abbrev1 = ecu._abbrev1 != null && au._abbrev1 != null
        ? "${ecu._abbrev1} / ${au._abbrev1}"
        : null;
    this._abbrev2 = ecu._abbrev2 != null && au._abbrev2 != null
        ? "${ecu._abbrev2}/${au._abbrev2}"
        : null;
    ;
