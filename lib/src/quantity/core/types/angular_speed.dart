...
class AngularSpeed extends Quantity {
  ...
  static const Dimensions angularSpeedDimensions =
      const Dimensions.constant(const {"Angle": 1, "Time": -1});

  /// The standard SI unit
  static final AngularSpeedUnits radiansPerSecond =
      new AngularSpeedUnits.angleTime(Angle.radians, Time.seconds);

  /// Accepted for use with the SI
  static final AngularSpeedUnits degreesPerSecond =
      new AngularSpeedUnits.angleTime(Angle.degrees, Time.seconds);

  AngularSpeed({dynamic rad_per_s, dynamic deg_per_s, double uncert: 0.0})
      : super(
            rad_per_s != null
                ? rad_per_s
                : (deg_per_s != null ? deg_per_s : 0.0),
            deg_per_s != null
                ? AngularSpeed.degreesPerSecond
                : AngularSpeed.radiansPerSecond,
            uncert);

  AngularSpeed._internal(conv)
      : super._dimensions(conv, AngularSpeed.angularSpeedDimensions);
  ...
  AngularSpeed.inUnits(value, AngularSpeedUnits units, [double uncert = 0.0])
      : super(value, units != null ? units : AngularSpeed.radiansPerSecond,
            uncert);

  const AngularSpeed.constant(Number valueSI,
      {AngularSpeedUnits units, num uncert: 0.0})
      : super.constant(
...
class AngularSpeedUnits extends AngularSpeed with Units {
  ...
  AngularSpeedUnits.angleTime(AngleUnits au, TimeUnits tu)
      : super._internal(au.valueSI * tu.valueSI) {
    ...
    this._abbrev1 = au._abbrev1 != null && tu._abbrev1 != null
        ? "${au._abbrev1} / ${tu._abbrev1}"
        : null;
    this._abbrev2 = au._abbrev2 != null && tu._abbrev2 != null
        ? "${au._abbrev2}${tu._abbrev2}"
        : null;
    ;
