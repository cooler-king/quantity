...
class ChargeDensity extends Quantity {
  ...
  ChargeDensity({dynamic C_per_m3, double uncert: 0.0})
      : super(C_per_m3 != null ? C_per_m3 : 0.0,
            ChargeDensity.coulombsPerCubicMeter, uncert);

  ChargeDensity._internal(conv)
      : super._dimensions(conv, ChargeDensity.electricChargeDensityDimensions);
  ...
  ChargeDensity.inUnits(value, ChargeDensityUnits units, [double uncert = 0.0])
      : super(
            value,
            units != null ? units : ChargeDensity.coulombsPerCubicMeter,
            uncert);

  const ChargeDensity.constant(Number valueSI,
      {ChargeDensityUnits units, num uncert: 0.0})
...
class ChargeDensityUnits extends ChargeDensity with Units {
  ...
  ChargeDensityUnits.chargeVolume(ChargeUnits ecu, VolumeUnits vu)
      : super._internal(ecu.valueSI * vu.valueSI) {
    this.name = "${ecu.name} per ${vu.singular}";
    this.singular = "${ecu.singular} per ${vu.singular}";
    this._convToMKS = ecu.valueSI * vu.valueSI;
    this._abbrev1 = ecu._abbrev1 != null && vu._abbrev1 != null
        ? "${ecu._abbrev1}/${vu._abbrev1}"
        : null;
    this._abbrev2 = ecu._abbrev2 != null && vu._abbrev2 != null
        ? "${ecu._abbrev2}/${vu._abbrev2}"
        : null;
    ;
