...
class AngularMomentum extends Quantity {
  ...
  static final AngularMomentumUnits jouleSecond =
      new AngularMomentumUnits.energyTime(Energy.joules, Time.seconds);

  AngularMomentum({dynamic Js, double uncert: 0.0})
      : super(Js != null ? Js : 0.0, AngularMomentum.jouleSecond, uncert);

  AngularMomentum._internal(conv)
      : super._dimensions(conv, AngularMomentum.angularMometumDimensions);

  /// Constructs a AngularMomentum based on the [value]
  /// and the conversion factor intrinsic to the passed [units].
  ///
  AngularMomentum.inUnits(value, AngularMomentumUnits units,
      [double uncert = 0.0])
      : super(
            value, units != null ? units : AngularMomentum.jouleSecond, uncert);

  const AngularMomentum.constant(Number valueSI,
      {AngularMomentumUnits units, num uncert: 0.0})
      : super.constant(
...
class AngularMomentumUnits extends AngularMomentum with Units {
  ...
  AngularMomentumUnits.energyTime(EnergyUnits eu, TimeUnits tu)
    ...
    this._abbrev1 = eu._abbrev1 != null && tu._abbrev1 != null
        ? "${eu._abbrev1} ${tu._abbrev1}"
        : null;
    this._abbrev2 = eu._abbrev2 != null && tu._abbrev2 != null
        ? "${eu._abbrev2}${tu._abbrev2}"
        : null;
    ;
