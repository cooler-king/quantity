...
class TemperatureInterval extends Quantity {
  ...
  static final TemperatureIntervalUnits kelvins =
      new TemperatureIntervalUnits("kelvins", null, "K", null, 1.0, true);
  ...
  TemperatureInterval({dynamic K, dynamic degC, double uncert: 0.0})
      : super(
            K != null ? K : (degC != null ? degC : 0.0),
            degC != null
                ? TemperatureInterval.degreesCelsius
                : TemperatureInterval.kelvins,
            uncert);

  TemperatureInterval._internal(conv)
      : super._dimensions(
            conv, TemperatureInterval.temperatureIntervalDimensions);

  /// Constructs a TemperatureInterval based on the [value]
  /// and the conversion factor intrinsic to the passed [units].
  ///
  TemperatureInterval.inUnits(value, TemperatureIntervalUnits units,
      [double uncert = 0.0])
      : super(
            value, units != null ? units : TemperatureInterval.kelvins, uncert);

  const TemperatureInterval.constant(Number valueSI,
      {TemperatureIntervalUnits units, num uncert: 0.0})
