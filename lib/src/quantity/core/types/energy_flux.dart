...
class EnergyFlux extends Quantity {
  ...
  static final EnergyFluxUnits wattsPerSquareMeter =
      new EnergyFluxUnits.powerArea(Power.watts, Area.squareMeters);

  EnergyFlux({dynamic W_per_m2, double uncert: 0.0})
      : super(W_per_m2 != null ? W_per_m2 : 0.0, EnergyFlux.wattsPerSquareMeter,
            uncert);

  EnergyFlux._internal(conv)
      : super._dimensions(conv, EnergyFlux.energyFluxDimensions);

  /// Constructs a EnergyFlux based on the [value]
  /// and the conversion factor intrinsic to the passed [units].
  ///
  EnergyFlux.inUnits(value, EnergyFluxUnits units, [double uncert = 0.0])
      : super(value, units != null ? units : EnergyFlux.wattsPerSquareMeter,
            uncert);

  const EnergyFlux.constant(Number valueSI,
      {EnergyFluxUnits units, num uncert: 0.0})
...
class EnergyFluxUnits extends EnergyFlux with Units {
  EnergyFluxUnits(String name, String abbrev1, String abbrev2, String singular,
      dynamic conv,
