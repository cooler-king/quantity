...
class MassFlowRate extends Quantity {
  ...
  static const Dimensions massFlowRateDimensions =
      const Dimensions.constant(const {"Mass": 1, "Time": -1});

  /// The standard SI unit.
  static final MassFlowRateUnits kilogramsPerSecond =
      new MassFlowRateUnits.massTime(Mass.kilograms, Time.seconds);

  MassFlowRate({dynamic kg_per_s, double uncert: 0.0})
      : super(kg_per_s != null ? kg_per_s : 0.0,
            MassFlowRate.kilogramsPerSecond, uncert);

  MassFlowRate._internal(conv)
      : super._dimensions(conv, MassFlowRate.massFlowRateDimensions);
  ...
  MassFlowRate.inUnits(value, MassFlowRateUnits units, [double uncert = 0.0])
      : super(value, units != null ? units : MassFlowRate.kilogramsPerSecond,
            uncert);

  const MassFlowRate.constant(Number valueSI,
      {MassFlowRateUnits units, num uncert: 0.0})
      : super.constant(
...
class MassFlowRateUnits extends MassFlowRate with Units {
  ...
  MassFlowRateUnits.massTime(MassUnits mu, TimeUnits tu)
      : super._internal(mu.valueSI * tu.valueSI) {
    this.name = "${mu.name} per ${tu.singular}";
    this.singular = "${mu.singular} per ${tu.singular}";
    this._convToMKS = mu.valueSI * tu.valueSI;
    this._abbrev1 = mu._abbrev1 != null && tu._abbrev1 != null
        ? "${mu._abbrev1} / ${tu._abbrev1}"
        : null;
    this._abbrev2 = mu._abbrev2 != null && tu._abbrev2 != null
        ? "${mu._abbrev2}${tu._abbrev2}"
        : null;
