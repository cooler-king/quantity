...
class MassDensity extends Quantity {
  ...
  static const Dimensions massDensityDimensions =
      const Dimensions.constant(const {"Mass": 1, "Length": -3});
  ...
  MassDensity({dynamic kg_per_m3, double uncert: 0.0})
      : super(kg_per_m3 != null ? kg_per_m3 : 0.0,
            MassDensity.kilogramsPerCubicMeter, uncert);

  MassDensity._internal(conv)
      : super._dimensions(conv, MassDensity.massDensityDimensions);

  /// Constructs a MassDensity based on the [value]
  /// and the conversion factor intrinsic to the passed [units].
  ///
  MassDensity.inUnits(value, MassDensityUnits units, [double uncert = 0.0])
      : super(value, units != null ? units : MassDensity.kilogramsPerCubicMeter,
            uncert);

  const MassDensity.constant(Number valueSI,
      {MassDensityUnits units, num uncert: 0.0})
      : super.constant(
...
class MassDensityUnits extends MassDensity with Units {
  MassDensityUnits(String name, String abbrev1, String abbrev2, String singular,
      dynamic conv,
  ...
  MassDensityUnits.massVolume(MassUnits mu, VolumeUnits vu)
      : super._internal(mu.valueSI * vu.valueSI) {
    this.name = "${mu.name} per ${vu.singular}";
    this.singular = "${mu.singular} per ${vu.singular}";
    this._convToMKS = mu.valueSI * vu.valueSI;
    this._abbrev1 = mu._abbrev1 != null && vu._abbrev1 != null
        ? "${mu._abbrev1} / ${vu._abbrev1}"
        : null;
    this._abbrev2 = mu._abbrev2 != null && vu._abbrev2 != null
        ? "${mu._abbrev2}${vu._abbrev2}"
        : null;
