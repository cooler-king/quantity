...
class EnergyDensity extends Quantity {
  ...
  EnergyDensity({dynamic J_per_m3, double uncert: 0.0})
      : super(J_per_m3 != null ? J_per_m3 : 0.0,
            EnergyDensity.joulesPerCubicMeter, uncert);

  EnergyDensity._internal(conv)
      : super._dimensions(conv, EnergyDensity.energyDensityDimensions);
  ...
  EnergyDensity.inUnits(value, EnergyDensityUnits units, [double uncert = 0.0])
      : super(value, units != null ? units : EnergyDensity.joulesPerCubicMeter,
            uncert);

  const EnergyDensity.constant(Number valueSI,
      {EnergyDensityUnits units, num uncert: 0.0})
      : super.constant(
...
class EnergyDensityUnits extends EnergyDensity with Units {
  ...
  EnergyDensityUnits.energyVolume(EnergyUnits eu, VolumeUnits vu)
      : super._internal(eu.valueSI * vu.valueSI) {
    this.name = "${eu.name} per ${vu.singular}";
    this.singular = "${eu.singular} per ${vu.singular}";
    this._convToMKS = eu.valueSI * vu.valueSI;
    this._abbrev1 = eu._abbrev1 != null && vu._abbrev1 != null
        ? "${eu._abbrev1} / ${vu._abbrev1}"
        : null;
    this._abbrev2 = eu._abbrev2 != null && vu._abbrev2 != null
        ? "${eu._abbrev2}/${vu._abbrev2}"
        : null;
    ;
