...
class LuminousIntensity extends Quantity {
  ...
  static const Dimensions luminousIntensityDimensions =
      const Dimensions.constant(const {"Intensity": 1});

  /// The standard SI unit.
  static final LuminousIntensityUnits candelas =
      new LuminousIntensityUnits("candelas", "cd", null, null, 1.0, true);

  LuminousIntensity({dynamic cd, double uncert: 0.0})
      : super(cd != null ? cd : 0.0, LuminousIntensity.candelas, uncert);

  LuminousIntensity._internal(conv)
      : super._dimensions(conv, LuminousIntensity.luminousIntensityDimensions);

  /// Constructs a LuminousIntensity based on the [value]
  /// and the conversion factor intrinsic to the passed [units].
  ///
  LuminousIntensity.inUnits(value, LuminousIntensityUnits units,
      [double uncert = 0.0])
      : super(
            value, units != null ? units : LuminousIntensity.candelas, uncert);

  const LuminousIntensity.constant(Number valueSI,
      {LuminousIntensityUnits units, num uncert: 0.0})
