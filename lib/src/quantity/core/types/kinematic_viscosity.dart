...
class KinematicViscosity extends Quantity {
  ...
  static const Dimensions kinematicViscosityDimensions =
      const Dimensions.constant(const {"Length": 2, "Time": -1});
  ...
  KinematicViscosity._internal(conv)
      : super._dimensions(
            conv, KinematicViscosity.kinematicViscosityDimensions);

  /// Constructs a KinematicViscosity based on the [value]
  /// and the conversion factor intrinsic to the passed [units].
  ///
  KinematicViscosity.inUnits(value, KinematicViscosityUnits units,
      [double uncert = 0.0])
      : super(
            value,
            units != null ? units : KinematicViscosity.metersSquaredPerSecond,
            uncert);

  const KinematicViscosity.constant(Number valueSI,
      {KinematicViscosityUnits units, num uncert: 0.0})
